	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"20px auto 60px","borderRadius":"20px","background":"#fff","width":"64%","position":"relative","height":"100%"}'>
		...
		<el-form ref="formRef" :model="form" class="add_form" label-width="120px" :rules="rules">
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="教学态度" prop="jiaoxuetaidu">
						<el-input class="list_inp" v-model="form.jiaoxuetaidu" placeholder="教学态度"
							 type="text" 							:readonly="!isAdd||disabledForm.jiaoxuetaidu?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="教学内容" prop="jiaoxueneirong">
						<el-input class="list_inp" v-model="form.jiaoxueneirong" placeholder="教学内容"
							 type="text" 							:readonly="!isAdd||disabledForm.jiaoxueneirong?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="教学进度" prop="jiaoxuejindu">
						<el-input class="list_inp" v-model="form.jiaoxuejindu" placeholder="教学进度"
							 type="text" 							:readonly="!isAdd||disabledForm.jiaoxuejindu?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="教学创新" prop="jiaoxuechuangxin">
						<el-input class="list_inp" v-model="form.jiaoxuechuangxin" placeholder="教学创新"
							 type="text" 							:readonly="!isAdd||disabledForm.jiaoxuechuangxin?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="教学水平" prop="jiaoxueshuiping">
						<el-input class="list_inp" v-model="form.jiaoxueshuiping" placeholder="教学水平"
							 type="text" 							:readonly="!isAdd||disabledForm.jiaoxueshuiping?true:false" />
					</el-form-item>
				</el-col>
				...
				<el-col :span="12">
					<el-form-item label="评估时间" prop="pinggushijian">
						<el-date-picker
							class="list_date"
							v-model="form.pinggushijian"
							format="YYYY-MM-DD HH:mm:ss"
							value-format="YYYY-MM-DD HH:mm:ss"
							type="datetime"
							style="width:100%;"
							:readonly="!isAdd||disabledForm.pinggushijian?true:false"
							placeholder="请选择评估时间" />
					</el-form-item>
				</el-col>
			</el-row>
...
<script setup>
	...
	const disabledForm = ref({
		...
		jiaoxuejindu : false,
		jiaoxuechuangxin : false,
		jiaoxueshuiping : false,
		xuehao : false,
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
			form.value.pinggushijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='jiaoxuetaidu'){
					form.value.jiaoxuetaidu = row[x];
					disabledForm.value.jiaoxuetaidu = true;
					continue;
				}
				if(x=='jiaoxueneirong'){
					form.value.jiaoxueneirong = row[x];
					disabledForm.value.jiaoxueneirong = true;
					continue;
				}
				if(x=='jiaoxuejindu'){
					form.value.jiaoxuejindu = row[x];
					disabledForm.value.jiaoxuejindu = true;
					continue;
				}
				if(x=='jiaoxuechuangxin'){
					form.value.jiaoxuechuangxin = row[x];
					disabledForm.value.jiaoxuechuangxin = true;
					continue;
				}
				if(x=='jiaoxueshuiping'){
					form.value.jiaoxueshuiping = row[x];
					disabledForm.value.jiaoxueshuiping = true;
					continue;
				}
				if(x=='xuehao'){
				...
				if(x=='pinggushijian'){
					form.value.pinggushijian = row[x];
					disabledForm.value.pinggushijian = true;
					continue;
				}
			}
			if(row){
		...
		context?.$http({
			url: `option/kechengxinxi/jiaoshigonghao`,
			method: 'get'
		}).then(res=>{
	...
	const jiaoshigonghaoChange=()=>{
		context?.$http({
			url: `follow/kechengxinxi/jiaoshigonghao?columnValue=` + form.value.jiaoshigonghao,
			method: 'get'
		}).then(res=>{
			//带出教师姓名字段
			...
			//带出课程名称字段
			if(res.data.data.kechengmingcheng){
				form.value.kechengmingcheng = res.data.data.kechengmingcheng
			}
			//带出课程类型字段
			if(res.data.data.kechengleixing){
				form.value.kechengleixing = res.data.data.kechengleixing
			}
		})
	}
	//提交
