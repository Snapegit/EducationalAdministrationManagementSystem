	...
	<div class="app-contain" :style='{"padding":"20px 0px","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"20px","flexWrap":"wrap","background":"#fff","display":"flex","width":"64%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">院系</div>
					<div  class="info_text" >{{detail.yuanxi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">选课时间</div>
					<div  class="info_text" >{{detail.xuankeshijian}}</div>
				</div>
				<div class="btn_view">
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('xueshengxuanke','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('xueshengxuanke','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('xueshengxuanke','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
