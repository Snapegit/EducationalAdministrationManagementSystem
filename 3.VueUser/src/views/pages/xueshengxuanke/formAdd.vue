			...
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="课程简介" prop="kechengjianjie">
						<el-input class="list_inp" v-model="form.kechengjianjie" placeholder="课程简介"
							 type="text" 							:readonly="!isAdd||disabledForm.kechengjianjie?true:false" />
					</el-form-item>
				</el-col>
				...
				<el-col :span="12">
					<el-form-item label="院系" prop="yuanxi">
						<el-input class="list_inp" v-model="form.yuanxi" placeholder="院系"
							 type="text" 							:readonly="!isAdd||disabledForm.yuanxi?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="选课时间" prop="xuankeshijian">
						<el-date-picker
							class="list_date"
							v-model="form.xuankeshijian"
							format="YYYY-MM-DD HH:mm:ss"
							value-format="YYYY-MM-DD HH:mm:ss"
							type="datetime"
							style="width:100%;"
							:readonly="!isAdd||disabledForm.xuankeshijian?true:false"
							placeholder="请选择选课时间" />
					</el-form-item>
				</el-col>
			</el-row>
...
<script setup>
	...
	//基础信息
	const tableName = 'xueshengxuanke'
	const formName = '学生选课'
	//基础信息
	const breadList = ref([{
		name: formName
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
			form.value.xuankeshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='yuanxi'){
					form.value.yuanxi = row[x];
					disabledForm.value.yuanxi = true;
					continue;
				}
				if(x=='xuankeshijian'){
					form.value.xuankeshijian = row[x];
					disabledForm.value.xuankeshijian = true;
					continue;
				}
				if(x=='crossuserid'){
		...
		context?.$http({
			...
			if(json.hasOwnProperty('yuanxi') && context?.$toolUtil.storageGet("frontRole")!="管理员"){
				form.value.yuanxi = json.yuanxi
				disabledForm.value.yuanxi = true;
			}
		})
	}
